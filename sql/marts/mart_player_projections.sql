-- =============================================================================
-- MART: mart_player_projections.sql
-- Replaces: sql/marts/mart_player_projections.sql
--
-- Fixes vs prior version:
--   - opp_tmf.def_sacks_std: now valid (added to mart_team_week_modeling)
--   - Injury dedup: reads raw_injuries directly with date_modified (not alias)
--   - mart_qb_projection: removed avg_air_yards_to_sticks (not in raw);
--     uses avg_intended_air_yards (confirmed present in raw_nextgen_stats)
--   - NGS passing/rushing cols will be NULL until those stat_type rows are
--     ingested — TRY_CAST in stg_nextgen_player_week handles this gracefully
-- =============================================================================
...
SELECT
  ...
  -- Usage STD (all from int_player_form — verified cols)
  ...
  -- Team context (from mart_team_week_modeling — verified cols)
  ...
  -- Opponent defensive context (from mart_team_week_modeling — verified cols)
  ...
  -- Injury (latest report per player/week)
...
-- Latest injury report per player/week
-- Reads raw_injuries directly using date_modified (actual column name).
-- Cannot use the view alias "report_date" inside a subquery ORDER BY.
LEFT JOIN (
...
SELECT
  ...
  -- QB lagged form (from int_player_form — verified cols)
  pf.attempts_per_game_std,
  pf.pass_yds_per_game_std,
  pf.pass_tds_per_game_std,
  pf.pass_epa_per_game_std,
  ...
  -- Team context (from int_qb_team_context — verified cols)
  qc.pass_rate,
  qc.team_pass_epa,
  ...
  -- NGS passing (NULL until passing stat_type rows exist in raw_nextgen_stats)
  ...
  -- Actuals
  qc.completions,
  qc.attempts                     AS actual_attempts,
  qc.passing_yards,
  qc.passing_tds,
  qc.interceptions,
  qc.sacks,
...
LEFT JOIN (
  SELECT player_id, season, week,
    AVG(cpoe)                   OVER w AS cpoe,
    AVG(aggressiveness)         OVER w AS aggressiveness,
    AVG(avg_time_to_throw)      OVER w AS avg_time_to_throw,
...
-- =============================================================================
-- mart_receiver_projection
-- Grain: game_id + player_id (WR + TE)
-- NGS receiving cols confirmed present in raw_nextgen_stats
-- =============================================================================
...
SELECT
  ...
  -- NGS receiving (confirmed present in raw_nextgen_stats)
  ngs.avg_separation,
  ngs.avg_cushion,
...
-- =============================================================================
-- mart_rusher_projection
-- Grain: game_id + player_id (RB)
-- NGS rushing cols return NULL (rushing rows not in raw_nextgen_stats this load)
-- =============================================================================
...
SELECT
  ...
  -- NGS rushing (NULL until rushing stat_type rows are ingested)
  ngs.rush_efficiency,
  ngs.stacked_box_pct,
  ngs.ryoe,
  ngs.ryoe_per_att,
...
LEFT JOIN (
  SELECT player_id, season, week,
    AVG(rush_efficiency) OVER w AS rush_efficiency,
    AVG(stacked_box_pct) OVER w AS stacked_box_pct,
    AVG(ryoe)            OVER w AS ryoe,
    AVG(ryoe_per_att)    OVER w AS ryoe_per_att,
    AVG(avg_time_to_los) OVER w AS avg_time_to_los
  FROM stg_nextgen_player_week
...
-- Grain: season + week + player_id (played games only)
-- =============================================================================
