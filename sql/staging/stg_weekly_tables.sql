...
-- Grain:   season + week + player_id + position
-- Purpose: Cleaned per-player weekly box score stats.
-- =============================================================================
...
-- Source:  raw_schedules + aggregated raw_pbp
-- Grain:   season + week + team (home and away as separate rows)
-- =============================================================================

CREATE OR REPLACE VIEW stg_team_week AS
WITH schedule_unpivot AS (
  -- Each game becomes two rows: one per team
  SELECT
    game_id,
    CAST(season AS INTEGER)  AS season,
    CAST(week   AS INTEGER)  AS week,
...
SELECT
  s.*,
  CAST(s.team_score AS INTEGER)         AS points_for,
  CAST(s.opp_score  AS INTEGER)         AS points_against,
  CAST(s.team_score AS INTEGER) - CAST(s.opp_score AS INTEGER) AS margin,
  CASE WHEN CAST(s.team_score AS INTEGER) > CAST(s.opp_score AS INTEGER) THEN 1
       WHEN CAST(s.team_score AS INTEGER) < CAST(s.opp_score AS INTEGER) THEN 0
       ELSE NULL END                    AS win_flag,
  CASE WHEN game_type = 'REG' THEN 'regular' ELSE 'postseason' END AS season_type

FROM schedule_unpivot s
...
-- Source:  raw_rosters_weekly
-- Grain:   season + week + team + player_id
-- =============================================================================
...
SELECT
  ...
  full_name,
  UPPER(position)                   AS position,
  CASE home_team WHEN 'OAK' THEN 'LV' WHEN 'SD' THEN 'LAC' WHEN 'STL' THEN 'LA' ELSE team END AS team,
  CAST(season  AS INTEGER)          AS season,
  CAST(week    AS INTEGER)          AS week,
  status,
  jersey_number,
...
-- Source:  raw_injuries
-- Grain:   season + week + team + player_id + report_primary_injury
-- =============================================================================
...
SELECT
  ...
  report_status,           -- OUT | Doubtful | Questionable | Limited | Full
  practice_primary_injury,
  practice_status,
  -- Numeric encoding for downstream injury burden scoring
...
SELECT
  ...
  TRY_CAST(avg_air_yards_differential AS DOUBLE) AS avg_air_yards_differential,
  TRY_CAST(aggressiveness             AS DOUBLE) AS aggressiveness,
  TRY_CAST(max_completed_air_distance AS DOUBLE) AS max_completed_air_distance,
  ...
  TRY_CAST(avg_intended_air_yards_rec AS DOUBLE) AS avg_intended_air_yards_rec,
...
-- Source:  raw_external_odds (placeholder — populated by separate ingestion)
